<?php

get_header();
the_post(); ?>

    <main class="content">
        <section class="section content--bg-img title-block default-title default-title-small bg-img-chemotherapy">
            <div class="wrapper">
                <div class="section__content">
                    <h3 class="text--18">
                        Главная / <span class="cell-for-text">Услуги / </span>
                        <span class="cell-for-text">Химиотерапия</span>
                    </h3>
                    <h1 class="text--48 default-title__text">
                        <?php the_title() ?>
                    </h1>
                    <h4 class="text--18 default-title__subtext">
                        <?php the_excerpt() ?>
                    </h4>
                </div>
            </div>
        </section>
        <section class="section section--default">
            <div class="wrapper">
                <div class="wrapper-920-left">
                    <div class="section--large-text">
                        <h3 class="title--small">
                            Описание
                        </h3>
                        <p class="text">
                            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Adipiscing quam enim at odio
                            quisque amet,
                            cursus purus. Quis sit nulla amet amet rhoncus aliquam luctus laoreet dignissim.
                            Adipiscing
                            convallis ut arcu nibh magna molestie consectetur scelerisque. Suspendisse sit arcu, et
                            tellus arcu
                            eu, pharetra congue quam. Magna quis mi nisl sed purus dolor ultricies sed pharetra.
                        </p>
                        <p class="text">
                            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Adipiscing quam enim at odio
                            quisque amet,
                            cursus purus. Quis sit nulla amet amet rhoncus aliquam luctus laoreet dignissim.
                            Adipiscing
                            convallis ut arcu nibh magna molestie consectetur scelerisque. Suspendisse sit arcu, et
                            tellus arcu
                            eu, pharetra congue quam. Magna quis mi nisl sed purus dolor ultricies sed pharetra.
                        </p>
                        <h3 class="title--small">
                            Противопоказание
                        </h3>
                        <p class="text">
                            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Adipiscing quam enim at odio
                            quisque amet,
                            cursus purus. Quis sit nulla amet amet rhoncus aliquam luctus laoreet dignissim.
                            Adipiscing
                            convallis ut arcu nibh magna molestie consectetur scelerisque. Suspendisse sit arcu, et
                            tellus arcu
                            eu, pharetra congue quam. Magna quis mi nisl sed purus dolor ultricies sed pharetra.
                        </p>
                        <p class="text">
                            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Adipiscing quam enim at odio
                            quisque amet,
                            cursus purus. Quis sit nulla amet amet rhoncus aliquam luctus laoreet dignissim.
                            Adipiscing
                            convallis ut arcu nibh magna molestie consectetur scelerisque. Suspendisse sit arcu, et
                            tellus arcu
                            eu, pharetra congue quam. Magna quis mi nisl sed purus dolor ultricies sed pharetra.
                        </p>
                        <h3 class="title--small">
                            Цены
                        </h3>
                        <p class="text">
                            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Adipiscing quam enim at odio
                            quisque amet,
                            cursus purus. Quis sit nulla amet amet rhoncus aliquam luctus laoreet dignissim.
                            Adipiscing
                            convallis ut arcu nibh magna molestie consectetur scelerisque. Suspendisse sit arcu, et
                            tellus arcu
                            eu, pharetra congue quam. Magna quis mi nisl sed purus dolor ultricies sed pharetra.
                        </p>
                        <p class="text">
                            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Adipiscing quam enim at odio
                            quisque amet,
                            cursus purus. Quis sit nulla amet amet rhoncus aliquam luctus laoreet dignissim.
                            Adipiscing
                            convallis ut arcu nibh magna molestie consectetur scelerisque. Suspendisse sit arcu, et
                            tellus arcu
                            eu, pharetra congue quam. Magna quis mi nisl sed purus dolor ultricies sed pharetra.
                        </p>
                        <div class="list-block">
                            <h3 class="title--small">
                                Этапы проведения
                            </h3>
                            <ul class="list text list-numeric">
                                <li class="list-item">
                                    <p class="list-item__title text--18">
                                        Название первого этапа
                                    </p>
                                    Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                                </li>
                                <li class="list-item">
                                    <p class="list-item__title text--18">
                                        Название первого этапа
                                    </p>
                                    Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                                </li>
                                <li class="list-item">
                                    <p class="list-item__title text--18">
                                        Название первого этапа
                                    </p>
                                    Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                                </li>
                                <li class="list-item">
                                    <p class="list-item__title text--18">
                                        Название первого этапа
                                    </p>
                                    Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                                </li>
                                <li class="list-item">
                                    <p class="list-item__title text--18">
                                        Название первого этапа
                                    </p>
                                    Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                                </li>
                            </ul>
                        </div>
                        <h3 class="title--small">
                            Как подготовиться
                        </h3>
                        <img src="../assets/images/preparation-for-treatment.png" class="text-big-img" alt="">
                        <p class="text">
                            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Adipiscing quam enim at odio
                            quisque amet,
                            cursus purus. Quis sit nulla amet amet rhoncus aliquam luctus laoreet dignissim.
                            Adipiscing
                            convallis ut arcu nibh magna molestie consectetur scelerisque. Suspendisse sit arcu, et
                            tellus arcu
                            eu, pharetra congue quam. Magna quis mi nisl sed purus dolor ultricies sed pharetra.
                        </p>
                        <h3 class="title--small">
                            Фото где проходят услуги
                        </h3>
                        <div class="simple-gallery">
                            <img class="simple-gallery__item " src="../assets/images/DSC_9438.png" alt="">
                            <img class="simple-gallery__item " src="../assets/images/sim-gal-img.png" alt="">
                            <img class="simple-gallery__item big-image" src="../assets/images/DSC_9746-1.png" alt="">
                            <img class="simple-gallery__item " src="../assets/images/DSC_9745.png" alt="">
                            <img class="simple-gallery__item " src="../assets/images/DSC_9687.png" alt="">
                        </div>
                        <?php get_template_part('template-parts/sections', 'contact-form-callback'); ?>
                    </div>
                </div>
            </div>

        </section>
    </main>
<?php
get_footer();

?>
