...
// add a setting
    ...
    $wp_customize->add_setting('your_theme_logo_ua');
    $wp_customize->add_setting('your_theme_color_logo_ua');
// Add a control to upload the hover logo
    ...
    $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'your_theme_logo_ua', array(
        'label' => 'Upload Logo UA',
        'section' => 'title_tagline', //this is the section where the custom-logo from WordPress is
        'settings' => 'your_theme_logo_ua',
        'priority' => 7 // show it just below the custom-logo
    )));
    $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'your_theme_color_logo_ua', array(
        'label' => 'Upload Color Logo UA',
        'section' => 'title_tagline', //this is the section where the custom-logo from WordPress is
        'settings' => 'your_theme_color_logo_ua',
        'priority' => 8 // show it just below the custom-logo
    )));
}
...
function faq_schema()
{
    ...
    "mainEntity": [';
        ...
        $count = 0; while ( $loop->have_posts() ) : $loop->the_post();
    ...
    "acceptedAnswer": {
      "@type": "Answer",
      "text": "'.esc_html(wp_strip_all_tags(get_the_content($post->ID))).'"
    }
  },';
...
add_action('wp_head', 'faq_schema', 1);
